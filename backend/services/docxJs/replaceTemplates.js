...
const {getDocAsWord, uploadDocxAndJsonToGoogleDrive} = require("../google/googleSheets");
// Extract placeholders using mammoth
async function extractPlaceholdersFromDocx(buffer) {
...
async function generateDocxFromTemplate(req) {
    const googleDocId = '1dpkWC_THDtITQ5WCg_OT4-ytMf3CdhH6IlL21JoGXjg';
    const response = await getDocAsWord(googleDocId);
    const zip = new PizZip(response.data);
    const templateBuffer = zip.generate({ type: "nodebuffer" });

    // const imgBuffer = await fs.readFile("/Users/or/IdeaProjects/gilgal/backend/services/docxJs/a.png");
    const imageFile =req.files.find((file) => file.fieldname === 'image');
    const imgBuffer = imageFile ? await fs.readFile(imageFile.path) : null;

    const tableNagar = JSON.parse(req.body.table_nagar);

    const patches = req.body.fields.reduce((acc, field) => {
    ...
    patches.table_nagar = {
        type: PatchType.DOCUMENT,
        children: [createReversedTableNagar(tableNagar)],
    };

    // Iterate through the placeholders to check for image placeholders
    for (const key of Object.keys(req.body)) {
        if (key.startsWith('img_')) {
            const imageBase64 = req.body[key]; // Get the Base64 image data
            patches[key] = {
                type: PatchType.PARAGRAPH,
                children: [
                    new ImageRun({
...
const generatedDocxBuffer= await patchDocument({
    outputType: "nodebuffer",
    data: templateBuffer,
    patches,
});

    const jsonData = JSON.stringify(req.body); // Convert req.body to JSON
    const projectDetails=JSON.parse(req.body.projectDetails);
   const fileIdsInDrive = await uploadDocxAndJsonToGoogleDrive(jsonData,generatedDocxBuffer,projectDetails);

    return generatedDocxBuffer;
}module.exports = { generateDocxFromTemplate ,extractPlaceholdersFromDocx};
